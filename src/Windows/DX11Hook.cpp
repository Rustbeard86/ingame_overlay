...
#define TRY_HOOK_FUNCTION(NAME)                                                                                        \
  do {                                                                                                                 \
    if (!HookFunc(std::make_pair<void**, void*>(&(void*&)_##NAME, (void*)&DX11Hook_t::_My##NAME))) {                   \
...
static inline HRESULT GetDeviceAndCtxFromSwapchain(IDXGISwapChain* pSwapChain, ID3D11Device** ppDevice,
...
bool DX11Hook_t::StartHook(std::function<void()> keyCombinationCallback, ToggleKey toggleKeys[], int toggleKeysCount,
...
bool DX11Hook_t::_CreateRenderTargets(IDXGISwapChain* pSwapChain) {
  ...
  if (!SUCCEEDED(_Device->CreateRenderTargetView(pBackBuffer, nullptr, &pRenderTargetView)) ||
...
void DX11Hook_t::_LoadResources() {
  ...
  for (size_t i = 0; i < loadParameterCount; ++i) {
    ...
    validResources.push_back({r, param.Data, param.Width, param.Height});
  }
...
ULONG STDMETHODCALLTYPE DX11Hook_t::_MyID3D11DeviceRelease(ID3D11Device* _this) {
  ...
  if (_this == inst->_Device) {
    INGAMEOVERLAY_INFO("ID3D11Device::Release: RefCount = {}, Our removal threshold = {}", result,
...
HRESULT STDMETHODCALLTYPE DX11Hook_t::_MyIDXGISwapChainResizeBuffers(IDXGISwapChain* _this, UINT BufferCount,
...
HRESULT STDMETHODCALLTYPE DX11Hook_t::_MyIDXGISwapChainResizeTarget(IDXGISwapChain* _this,
...
HRESULT STDMETHODCALLTYPE DX11Hook_t::_MyIDXGISwapChain1Present1(IDXGISwapChain1* _this, UINT SyncInterval, UINT Flags,
...
DX11Hook_t::DX11Hook_t()
    : _Hooked(false), _WindowsHooked(false), _UsesDXVK(false), _DeviceReleasing(0), _Device(nullptr),
      _HookDeviceRefCount(0), _HookState(OverlayHookState::Removing), _DeviceContext(nullptr),
      _RenderTargetView(nullptr), _ImGuiFontAtlas(nullptr), _ID3D11DeviceRelease(nullptr),
...
void DX11Hook_t::LoadFunctions(decltype(_ID3D11DeviceRelease) releaseFcn, decltype(_IDXGISwapChainPresent) presentFcn,
