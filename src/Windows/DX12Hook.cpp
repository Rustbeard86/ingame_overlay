...
#define TRY_HOOK_FUNCTION(NAME)                                                                                        \
  do {                                                                                                                 \
    if (!HookFunc(std::make_pair<void**, void*>(&(void*&)_##NAME, (void*)&DX12Hook_t::_My##NAME))) {                   \
...
static_assert(sizeof(D3D12_GPU_DESCRIPTOR_HANDLE::ptr) == sizeof(ImTextureID),
              "sizeof(D3D12_GPU_DESCRIPTOR_HANDLE::ptr) must match sizeof(ImTextureID)");
...
static InGameOverlay::ScreenshotDataFormat_t RendererFormatToScreenshotFormat(DXGI_FORMAT format) {
  switch (format) {
    ...
    case DXGI_FORMAT_B8G8R8X8_UNORM:
      return InGameOverlay::ScreenshotDataFormat_t::B8G8R8X8;
    case DXGI_FORMAT_R10G10B10A2_UNORM:
      return InGameOverlay::ScreenshotDataFormat_t::R10G10B10A2;
    case DXGI_FORMAT_B5G6R5_UNORM:
      return InGameOverlay::ScreenshotDataFormat_t::B5G6R5;
    case DXGI_FORMAT_B5G5R5A1_UNORM:
      return InGameOverlay::ScreenshotDataFormat_t::B5G5R5A1;
...
bool DX12Hook_t::StartHook(std::function<void()> keyCombinationCallback, ToggleKey toggleKeys[], int toggleKeysCount,
...
DX12Hook_t::ShaderRessourceView_t DX12Hook_t::_GetFreeShaderRessourceViewFromHeap(uint32_t heapIndex) {
  ...
  for (uint32_t usedIndex = 0; usedIndex < heap.HeapBitmap.size(); ++usedIndex) {
    if (!heap.HeapBitmap[usedIndex]) {
      ...
      ++heap.UsedHeap;
      result.CpuHandle.ptr =
          _ShaderResourceViewHeapDescriptors[heapIndex].Heap->GetCPUDescriptorHandleForHeapStart().ptr +
          inc * usedIndex;
      result.GpuHandle.ptr =
          _ShaderResourceViewHeapDescriptors[heapIndex].Heap->GetGPUDescriptorHandleForHeapStart().ptr +
          inc * usedIndex;
...
ID3D12CommandQueue* DX12Hook_t::_FindCommandQueueFromSwapChain(IDXGISwapChain* pSwapChain) {
  ...
  if (_CommandQueueOffset != 0) {
    pCommandQueue =
        *reinterpret_cast<ID3D12CommandQueue**>(reinterpret_cast<uintptr_t>(pSwapChain) + _CommandQueueOffset);
  } else if (_CommandQueueOffsetRetries <= MaxRetries) {
    if (++_CommandQueueOffsetRetries >= MaxRetries) {
      INGAMEOVERLAY_INFO(
          "Failed to find IDXGISwapChain::ppCommandQueue, fallback to ID3D12CommandQueue::ExecuteCommandLists");
    }
  } else {
...
void DX12Hook_t::_UpdateHookDeviceRefCount() {
  switch (_HookState) {
    case OverlayHookState::Removing:
      _HookDeviceRefCount = +1 // _Device->AddRef
          ;
      break;
    // ImGui:
    //   PipelineState
    ...
    case OverlayHookState::Ready:
      _HookDeviceRefCount =
          ...
          + 1                               // ImageCommandList
          + _ShaderResourceViewHeapDescriptors.size() + _ImageResourcesToRelease.size() +
          std::count_if(_ImageResources.begin(), _ImageResources.end(),
                        [](std::shared_ptr<RendererTexture_t> tex) {
                          return tex->LoadStatus == RendererTextureStatus_e::Loaded;
                        }) +
...
bool DX12Hook_t::_CreateRenderTargets(IDXGISwapChain* pSwapChain) {
...

  {
    ...
    for (UINT i = 0; i < bufferCount; ++i) {
      ...
      if (_Device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&frame.CommandAllocator)) !=
              S_OK ||
      ...
      if (_Device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, frame.CommandAllocator, NULL,
                                     IID_PPV_ARGS(&frame.CommandList)) != S_OK ||
...
bool DX12Hook_t::_CreateImageObjects() {
  ...
  hr = _Device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, _ImageCommandAllocator, NULL,
                                  IID_PPV_ARGS(&_ImageCommandList));
  if (!SUCCEEDED(hr))
    goto on_fail;
...
void DX12Hook_t::_PrepareForOverlay(IDXGISwapChain* pSwapChain, ID3D12CommandQueue* pCommandQueue, UINT flags) {
  ...
  if (ImGui_ImplDX12_NewFrame() && WindowsHook_t::Inst()->PrepareForOverlay(sc_desc.OutputWindow)) {
    ...
    ImGui::Render();
    ImGui_ImplDX12_RenderDrawData(ImGui::GetDrawData(), frame.CommandList, &_ShaderResourceViewHeapDescriptors[0].Heap,
                                  (int)_ShaderResourceViewHeapDescriptors.size(), ShaderResourceViewHeap_t::HeapSize);
...
void DX12Hook_t::_LoadResources() {
  ...
  for (size_t i = 0; i < loadParameterCount; ++i) {
    ...
    validResources.push_back(
        ValidTexture_t{std::static_pointer_cast<DX12Texture_t>(r), param.Data, param.Width, param.Height});
  }
  ...
  if (!validResources.empty()) {
    ...
    for (auto& tex : validResources) {
      UINT pitch =
          (tex.Width * 4 + D3D12_TEXTURE_DATA_PITCH_ALIGNMENT - 1u) & ~(D3D12_TEXTURE_DATA_PITCH_ALIGNMENT - 1u);
    ...
    for (size_t i = 0; i < validResources.size(); ++i) {
      ...
      for (uint32_t y = 0; y < tex.Height; ++y) {
        memcpy((uint8_t*)mapped + offset + y * pitch, (const uint8_t*)tex.Data + y * tex.Width * 4, tex.Width * 4);
      }
...
void DX12Hook_t::_HandleScreenshot(DX12Frame_t& frame) {
  ...
  hr = _Device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, pCommandAlloc, nullptr,
  ...
  hr = _Device->CreateCommittedResource(&readBackHeapProperties, D3D12_HEAP_FLAG_NONE, &bufferDesc,
                                        D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS(&pStaging));
...
ULONG STDMETHODCALLTYPE DX12Hook_t::_MyID3D12DeviceRelease(IUnknown* _this) {
  ...
  if (_this == inst->_Device) {
    INGAMEOVERLAY_INFO("ID3D12Device::Release: RefCount = {}, Our removal threshold = {}", result,
                       inst->_HookDeviceRefCount);
...
HRESULT STDMETHODCALLTYPE DX12Hook_t::_MyIDXGISwapChainResizeBuffers(IDXGISwapChain* _this, UINT BufferCount,
  ...
  if (createRenderTargets) {
    inst->_ResetRenderState(inst->_CreateRenderTargets(_this) ? OverlayHookState::Ready : OverlayHookState::Removing);
  }

  return r;
}

HRESULT STDMETHODCALLTYPE DX12Hook_t::_MyIDXGISwapChainResizeTarget(IDXGISwapChain* _this,
...
HRESULT STDMETHODCALLTYPE DX12Hook_t::_MyIDXGISwapChain1Present1(IDXGISwapChain1* _this, UINT SyncInterval, UINT Flags,
...
HRESULT STDMETHODCALLTYPE DX12Hook_t::_MyIDXGISwapChain3ResizeBuffers1(IDXGISwapChain3* _this, UINT BufferCount,
  ...
  auto r = (_this->*inst->_IDXGISwapChain3ResizeBuffers1)(BufferCount, Width, Height, Format, SwapChainFlags,
...
void STDMETHODCALLTYPE DX12Hook_t::_MyID3D12CommandQueueExecuteCommandLists(ID3D12CommandQueue* _this,
...
DX12Hook_t::DX12Hook_t()
    : _Hooked(false), _WindowsHooked(false), _DeviceReleasing(0), _CommandQueueOffsetRetries(0), _CommandQueueOffset(0),
      _CommandQueue(nullptr), _Device(nullptr), _HookDeviceRefCount(0), _HookState(OverlayHookState::Removing),
      _RenderTargetViewDescriptorHeap(nullptr), _ImageEvent(nullptr), _ImageFenceValue(0), _ImageFence(nullptr),
      _ImageCommandQueue(nullptr), _ImageCommandAllocator(nullptr), _ImageCommandList(nullptr),
      _ImGuiFontTextureId(ShaderRessourceView_t::InvalidId), _ImGuiFontAtlas(nullptr), _ID3D12DeviceRelease(nullptr),
      _IDXGISwapChainPresent(nullptr), _IDXGISwapChainResizeBuffers(nullptr), _IDXGISwapChainResizeTarget(nullptr),
      _IDXGISwapChain3ResizeBuffers1(nullptr), _ID3D12CommandQueueExecuteCommandLists(nullptr),
...
void DX12Hook_t::LoadFunctions(decltype(_ID3D12DeviceRelease) releaseFcn, decltype(_IDXGISwapChainPresent) presentFcn,
