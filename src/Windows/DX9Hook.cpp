...
#define TRY_HOOK_FUNCTION(NAME)                                                                                        \
  do {                                                                                                                 \
    if (!HookFunc(std::make_pair<void**, void*>(&(void*&)_##NAME, (void*)&DX9Hook_t::_My##NAME))) {                    \
...
static InGameOverlay::ScreenshotDataFormat_t RendererFormatToScreenshotFormat(D3DFORMAT format) {
  switch (format) {
    case D3DFMT_R8G8B8:
      return InGameOverlay::ScreenshotDataFormat_t::R8G8B8;
    case D3DFMT_X8R8G8B8:
      return InGameOverlay::ScreenshotDataFormat_t::X8R8G8B8;
    case D3DFMT_A8R8G8B8:
      return InGameOverlay::ScreenshotDataFormat_t::A8R8G8B8;
    case D3DFMT_R5G6B5:
      return InGameOverlay::ScreenshotDataFormat_t::R5G6B5;
    case D3DFMT_X1R5G5B5:
      return InGameOverlay::ScreenshotDataFormat_t::X1R5G5B5;
    case D3DFMT_A1R5G5B5:
      return InGameOverlay::ScreenshotDataFormat_t::A1R5G5B5;
    case D3DFMT_A2R10G10B10:
      return InGameOverlay::ScreenshotDataFormat_t::A2R10G10B10;
    case D3DFMT_A2B10G10R10:
      return InGameOverlay::ScreenshotDataFormat_t::A2B10G10R10;
    default:
      return InGameOverlay::ScreenshotDataFormat_t::Unknown;
  }
}

bool DX9Hook_t::StartHook(std::function<void()> keyCombinationCallback, ToggleKey toggleKeys[], int toggleKeysCount,
                          /*ImFontAtlas* */ void* imguiFontAtlas) {
  if (!_Hooked) {
    if (_IDirect3DDevice9Release == nullptr || _IDirect3DDevice9Reset == nullptr ||
  ...
  switch (_HookState) {
    ...
    case OverlayHookState::Ready:
      _HookDeviceRefCount =
          ...
          std::count_if(_ImageResources.begin(), _ImageResources.end(), [](std::shared_ptr<RendererTexture_t> tex) {
...
void DX9Hook_t::_LoadResources() {
  ...
  if (!validResources.empty()) {
    ...
    for (size_t i = 0; i < validResources.size(); ++i) {
      ...
      dx9Tex = nullptr;
      _Device->CreateTexture(tex.Width, tex.Height, 1, D3DUSAGE_DYNAMIC, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &dx9Tex,
                             nullptr);
        ...
        if (SUCCEEDED(dx9Tex->LockRect(0, &rect, nullptr, D3DLOCK_DISCARD))) {
          ...
          for (uint32_t i = 0; i < tex.Height; ++i) {
            for (uint32_t j = 0; j < tex.Width; ++j) {
              ...
              reinterpret_cast<uint32_t*>(texture_bits)[j] =
...
void DX9Hook_t::_HandleScreenshot() {
  ...
  hr = _Device->CreateOffscreenPlainSurface(desc.Width, desc.Height, desc.Format, D3DPOOL_SYSTEMMEM, &cpuSurface,
                                            nullptr);
...
ULONG STDMETHODCALLTYPE DX9Hook_t::_MyIDirect3DDevice9Release(IDirect3DDevice9* _this) {
  ...
  if (_this == inst->_Device) {
    INGAMEOVERLAY_INFO("IDirect3DDevice9::Release: RefCount = {}, Our removal threshold = {}", result,
...
HRESULT STDMETHODCALLTYPE DX9Hook_t::_MyIDirect3DDevice9Reset(IDirect3DDevice9* _this,
  ...
  if (createRenderTargets) {
    inst->_ResetRenderState(ImGui_ImplDX9_CreateDeviceObjects() ? OverlayHookState::Ready : OverlayHookState::Removing);
  }

  return r;
}

HRESULT STDMETHODCALLTYPE DX9Hook_t::_MyIDirect3DDevice9Present(IDirect3DDevice9* _this, CONST RECT* pSourceRect,
...
HRESULT STDMETHODCALLTYPE DX9Hook_t::_MyIDirect3DDevice9ExPresentEx(IDirect3DDevice9Ex* _this, CONST RECT* pSourceRect,
                                                                    CONST RECT* pDestRect, HWND hDestWindowOverride,
                                                                    CONST RGNDATA* pDirtyRegion, DWORD dwFlags) {
  INGAMEOVERLAY_INFO("IDirect3DDevice9Ex::PresentEx");
  auto inst = DX9Hook_t::Inst();
  inst->_PrepareForOverlay(_this, hDestWindowOverride);
  return (_this->*inst->_IDirect3DDevice9ExPresentEx)(pSourceRect, pDestRect, hDestWindowOverride, pDirtyRegion,
                                                      dwFlags);
}

HRESULT STDMETHODCALLTYPE DX9Hook_t::_MyIDirect3DDevice9ExResetEx(IDirect3DDevice9Ex* _this,
                                                                  D3DPRESENT_PARAMETERS* pPresentationParameters,
...
HRESULT STDMETHODCALLTYPE DX9Hook_t::_MyIDirect3DSwapChain9SwapChainPresent(
    IDirect3DSwapChain9* _this, CONST RECT* pSourceRect, CONST RECT* pDestRect, HWND hDestWindowOverride,
    CONST RGNDATA* pDirtyRegion, DWORD dwFlags) {
  ...
  return (_this->*inst->_IDirect3DSwapChain9SwapChainPresent)(pSourceRect, pDestRect, hDestWindowOverride, pDirtyRegion,
                                                              dwFlags);
}

DX9Hook_t::DX9Hook_t()
    : _Hooked(false), _WindowsHooked(false), _UsesDXVK(false), _LastWindow(nullptr), _DeviceReleasing(0),
      _Device(nullptr), _HookDeviceRefCount(0), _HookState(OverlayHookState::Removing), _ImGuiFontAtlas(nullptr),
      _IDirect3DDevice9Release(nullptr), _IDirect3DDevice9Present(nullptr), _IDirect3DDevice9Reset(nullptr),
      _IDirect3DDevice9ExPresentEx(nullptr), _IDirect3DDevice9ExResetEx(nullptr),
...
void DX9Hook_t::LoadFunctions(decltype(_IDirect3DDevice9Release) ReleaseFcn,
                              decltype(_IDirect3DDevice9Present) PresentFcn, decltype(_IDirect3DDevice9Reset) ResetFcn,
