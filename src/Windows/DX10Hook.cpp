...
#define TRY_HOOK_FUNCTION(NAME)                                                                                        \
  do {                                                                                                                 \
    if (!HookFunc(std::make_pair<void**, void*>(&(void*&)_##NAME, (void*)&DX10Hook_t::_My##NAME))) {                   \
...
bool DX10Hook_t::StartHook(std::function<void()> keyCombinationCallback, ToggleKey toggleKeys[], int toggleKeysCount,
...
ULONG STDMETHODCALLTYPE DX10Hook_t::_MyID3D10DeviceRelease(ID3D10Device* _this) {
  ...
  if (_this == inst->_Device) {
    INGAMEOVERLAY_INFO("ID3D10Device::Release: RefCount = {}, Our removal threshold = {}", result,
...
HRESULT STDMETHODCALLTYPE DX10Hook_t::_MyIDXGISwapChainResizeBuffers(IDXGISwapChain* _this, UINT BufferCount,
...
HRESULT STDMETHODCALLTYPE DX10Hook_t::_MyIDXGISwapChainResizeTarget(IDXGISwapChain* _this,
...
HRESULT STDMETHODCALLTYPE DX10Hook_t::_MyIDXGISwapChain1Present1(IDXGISwapChain1* _this, UINT SyncInterval, UINT Flags,
...
DX10Hook_t::DX10Hook_t()
    : _Hooked(false), _WindowsHooked(false), _UsesDXVK(false), _DeviceReleasing(0), _Device(nullptr),
      _HookDeviceRefCount(0), _HookState(OverlayHookState::Removing), _RenderTargetView(nullptr),
      _ImGuiFontAtlas(nullptr), _ID3D10DeviceRelease(nullptr), _IDXGISwapChainPresent(nullptr),
      _IDXGISwapChainResizeBuffers(nullptr), _IDXGISwapChainResizeTarget(nullptr), _IDXGISwapChain1Present1(nullptr) {}
...
void DX10Hook_t::LoadFunctions(decltype(_ID3D10DeviceRelease) releaseFcn, decltype(_IDXGISwapChainPresent) presentFcn,
