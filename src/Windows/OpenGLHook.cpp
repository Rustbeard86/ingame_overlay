...
#define TRY_HOOK_FUNCTION_OR_FAIL(NAME)                                                                                \
  do {                                                                                                                 \
    if (!HookFunc(std::make_pair<void**, void*>(&(void*&)_##NAME, (void*)&OpenGLHook_t::_My##NAME))) {                 \
...
bool OpenGLHook_t::StartHook(std::function<void()> keyCombinationCallback, ToggleKey toggleKeys[], int toggleKeysCount,
...
OpenGLHook_t::OpenGLHook_t()
    : _Hooked(false), _WindowsHooked(false), _Initialized(false), _HookState(OverlayHookState::Removing),
      _LastWindow(nullptr), _ImGuiFontAtlas(nullptr), _WGLSwapBuffers(nullptr) {}
